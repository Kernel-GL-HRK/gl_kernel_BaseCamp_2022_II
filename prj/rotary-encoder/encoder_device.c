...
#define PROC_FILE_NAME "encoder_info"
...
static irqreturn_t encoder_irq_handler(int irq, void *dev_id)
{
	...
	// mdelay(DEBOUNCE_DELAY_MS); 

	if (ktime_to_ms(ktime_sub(interrupt_time, last_interrupt_time)) >= DEBOUNCE_DELAY_MS) {
	a_state = gpio_get_value(ENCODER_GPIO_A);
    b_state = gpio_get_value(ENCODER_GPIO_B);
...
static ssize_t dev_read(struct file *filep, char *buffer, size_t len, loff_t *offset)
{
    ...
    if (len < num_bytes){
		pr_err("chrdev: copy to user failed\n");
		return -EINVAL;
	}
...
static int __init chrdev_init(void)
{
    ...
    // Устанавливаем GPIO как входы
    gpio_direction_input(ENCODER_GPIO_A);
    gpio_direction_input(ENCODER_GPIO_B);
    
    // Регистрируем обработчик прерывания для энкодера
    irq = gpio_to_irq(ENCODER_GPIO_A);
    if (request_irq(irq, encoder_irq_handler, IRQF_TRIGGER_FALLING, "encoder_irq", NULL)) {
